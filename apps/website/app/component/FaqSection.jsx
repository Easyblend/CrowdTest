            ...
            <div className="mt-8">
                {faqsData.map((faq, index) => (
                    <div className="border-b border-slate-300 dark:border-purple-900 py-4 cursor-pointer w-full" key={index} onClick={() => setOpenIndex(openIndex === index ? null : index)}>
                        ...
                        <p className={`text-sm text-slate-600 dark:text-slate-300 transition-all duration-300 ease-in-out max-w-xl ${openIndex === index ? "opacity-100 max-h-125 translate-y-0 pt-4" : "opacity-0 max-h-0 overflow-hidden -translate-y-2"}`} >
                            {faq.answer}
                        </p>
                    </div>
                ))}
